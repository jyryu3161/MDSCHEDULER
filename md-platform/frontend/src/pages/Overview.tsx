...
import { dashboardApi, designApi, gpuApi, normalizeError } from "../api";
import { Card, ErrorBanner, ProgressBar, Spinner, StatCard } from "../components";
import { formatGb, formatNumber } from "../format";
import type { DashboardSummary, DesignJob, GpuStatus } from "../types";
    ...
    const refresh = async () => {
      try {
        const [s, d, g] = await Promise.all([
          dashboardApi.summary(), designApi.list(), gpuApi.list(),
        ]);
        if (!mounted.current) return;
        setSummary(s); setDesigns(d); setGpus(g); setError(null);
      } catch (err) {
